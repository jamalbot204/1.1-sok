...
import { Key, Trash2, ShieldAlert, Award, RefreshCw, UserCheck, HardDrive, Smartphone, Share2, MapPin } from 'lucide-react';
...
export const Settings: React.FC = () => {
  ...
  return (
    <div className="flex-1 p-5 pb-8 space-y-5 flex flex-col justify-start overflow-y-auto min-h-0">
      ...
      {/* CUSTOM GEMINI API KEY INTEGRATOR */}
      <div className="bg-slate-900/30 border border-slate-800/80 rounded-3xl p-5 space-y-4">
        <h2 className="text-xs font-bold text-slate-300 select-none flex items-center">
          <Key className="w-4 h-4 text-sky-400 mr-1.5 ml-1.5" />
          مفتاح برمجة التطبيقات (Gemini API Key)
        </h2>
        
        <p className="text-[10px] text-slate-400 leading-relaxed text-right select-none">
          يتيح لك المساعد "مظبوط" تحليل وجبات الطعام والجرعات. يتم الحفظ بمحلي آمن بجهازك. إذا تركت الحقل فارغاً سيعتمد التطبيق على مفتاح السيرفر التلقائي المدمج.
        </p>
        ...
        <form onSubmit={handleSaveKey} className="space-y-3.5 pt-1">
          <div className="relative">
            <input
              ...
              placeholder="••••••••••••••••••••••••••••"
              className="w-full bg-slate-950 border border-slate-800 focus:border-sky-500 rounded-xl px-4 py-3 text-xs text-slate-300 font-mono text-center tracking-widest placeholder:tracking-normal placeholder:font-sans"
            />
          </div>
          ...
          <div className="flex gap-2">
            {geminiApiKey && (
              <button
                ...
                className="px-3 bg-slate-950 text-rose-400 border border-slate-800 rounded-xl hover:bg-rose-500/10 transition-colors active:scale-95"
              >
                حذف
              </button>
            )}
            <button
              type="submit"
              className="flex-1 py-2.5 bg-sky-500 hover:bg-sky-600 text-slate-950 font-extrabold text-xs rounded-xl transition-all active:scale-95 shadow-md shadow-sky-500/5"
            >
              حفظ المفتاح
            </button>
          </div>
        ...
        {showSaveFeedback && (
          <div className="bg-sky-500/10 border border-sky-500/20 p-2.5 text-center text-[10px] text-sky-400 font-bold rounded-xl animate-pulse">
            تم تحديث مفتاح برمجة السكري الذكي محلياً ✓
          </div>
        )}
      </div>
