...
export const ChatInputArea = React.memo(({ isTyping, onSendMessage, onStopGeneration, inputAreaRef }: ChatInputAreaProps) => {
  ...
  const startListening = async () => {
    ...
    try {
      // Prompt permissions natively or via core webview stream
      ...
      setSpeechError('الرجاء السماح بصلاحية استخدام الميكروفون من إعدادات الهاتف والموقع.');
      setTimeout(() => setSpeechError(null), 4000);
      return;
    }
    ...
    try {
      ...
      recognition.lang = 'ar-SY'; // Set Arabic - Syria locale standard for dialect coverage
      ...
      recognition.onerror = (event: any) => {
        ...
        if (event.error === 'not-allowed') {
          setSpeechError('صلاحية الميكروفون غير متوفرة أو تم رفضها.');
        ...
        <form onSubmit={handleSubmit} className="w-full flex flex-col bg-slate-950 border border-slate-800/80 rounded-3xl p-1.5 focus-within:border-sky-500 transition-all">
          ...
          <div className="w-full">
            <input
              ...
              disabled={isTyping}
              placeholder={isTyping ? "الرجاء المهل ثانية..." : "اكتب هنا للاستشارة الذكية..."}
              className="w-full bg-transparent border-0 outline-none focus:outline-none focus:ring-0 px-3 py-2 text-xs text-slate-200 placeholder:text-slate-600 font-medium text-right focus:border-transparent focus:ring-transparent focus:outline-none"
              dir="rtl"
            />
          </div>
          ...
          <div className="flex items-center justify-between gap-2 px-1 focus-within:border-sky-500 mt-1 pt-1.5 border-t border-slate-900/60">
            ...
            <div className="flex items-center gap-1.5" dir="rtl">
              <button
                ...
                className="p-2 bg-slate-900 border border-slate-800/60 hover:border-sky-500/30 text-slate-400 hover:text-sky-400 rounded-xl active:scale-90 transition-colors shrink-0"
                title="إرفاق صورة"
              >
              ...
              <button
                ...
                className={`p-2 border rounded-xl transition-all active:scale-95 shrink-0 relative flex items-center justify-center ${
                  isListening
